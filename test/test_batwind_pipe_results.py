...
import json
from batwind.pipelines.batwind_pipe_results import main
...
def _write_state(tmp_path):
        ...
        "computed_results": {
            "alpha.plt": {
                ...
                "letter_counts": {
                    ...
                    "source": {
                        "module": "batwind.pipelines.dummy_pipeline",
    ...
    path = tmp_path / "batwind-pipe.dummy.processed.json"
    path.write_text(json.dumps(payload, indent=2))
    return path


def test_batwind_pipe_results_lists_fields(tmp_path, capsys):
...
def test_batwind_pipe_results_lists_files(tmp_path, capsys):
    path = _write_state(tmp_path)
...
def test_batwind_pipe_results_field_query_value_only(tmp_path, capsys):
    path = _write_state(tmp_path)
...
def test_batwind_pipe_results_default_dump_all(tmp_path, capsys):
    path = _write_state(tmp_path)
