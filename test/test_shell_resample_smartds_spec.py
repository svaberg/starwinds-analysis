...
def test_sample_spherical_shells_returns_structured_smartds_with_free_and_bound_coordinates():
    ...
    - free coordinates (`R`, `polar`, `azimuth`) are populated
    ...
    polar = np.array(shell_ds.variable("polar [rad]"))
    azimuth = np.array(shell_ds.variable("azimuth [rad]"))
    ...
    assert polar.shape == expected_shape
    assert azimuth.shape == expected_shape
    assert sampled.shape == expected_shape

    r_direct = np.sqrt(x * x + y * y + z * z)
    np.testing.assert_allclose(r, r_direct, rtol=0, atol=1e-10)

    polar_direct = np.arccos(np.clip(z / r_direct, -1.0, 1.0))
    np.testing.assert_allclose(polar, polar_direct, rtol=0, atol=1e-10)

    azimuth_direct = np.arctan2(y, x)
    d_azimuth = (azimuth - azimuth_direct + np.pi) % (2.0 * np.pi) - np.pi
    np.testing.assert_allclose(d_azimuth, 0.0, rtol=0, atol=1e-10)
    ...
    for name in ("R [R]", "polar [rad]", "azimuth [rad]"):
        assert name in shell_ds_all.variables
