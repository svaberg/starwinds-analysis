"""CLI parser for per-pipeline `batwind-pipe.<pipeline>.processed.json` results."""
...
from batwind.pipelines.recorder import load_state_payload
...
def build_parser() -> argparse.ArgumentParser:
    """
    Build the `batwind-pipe-results` CLI parser.
    Used by: `batwind/pipelines/batwind_pipe_results.py`
    """
    parser = argparse.ArgumentParser(description="Inspect batwind-pipe.<pipeline>.processed.json results.")
    parser.add_argument(
        "--state",
        default="batwind-pipe.dummy.processed.json",
        help="Path to batwind-pipe state JSON (default: ./batwind-pipe.dummy.processed.json).",
    )
    parser.add_argument(
        "--list-files",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    """
    CLI entry point for `batwind-pipe-results`.
    Used by: CLI (`batwind-pipe-results`), `test/test_batwind_pipe_results.py`
    """
    parser = build_parser()
