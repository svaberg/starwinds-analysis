...
def name_letter_counts(name: str) -> tuple[int, int]:
    ...
    emit_log.debug(
        "emit letter_counts",
        extra={"sw_emit_key": "letter_counts", "sw_emit_value": {"vowels": vowels, "consonants": consonants}},
    )
...
def name_profile_payload(name: str) -> tuple[float, str, list[int]]:
    ...
    emit_log.debug("emit name_vowel_fraction", extra={"sw_emit_key": "name_vowel_fraction", "sw_emit_value": vowel_fraction})
    emit_log.debug("emit name_dominance", extra={"sw_emit_key": "name_dominance", "sw_emit_value": dominance})
    emit_log.debug("emit name_shape", extra={"sw_emit_key": "name_shape", "sw_emit_value": shape})
