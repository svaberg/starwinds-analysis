...
- `starwinds_analysis/physics/curve.py` — **Debt**. This merged curve-diagnostics file now consumes sampled curves only and no longer constructs orbit geometry. Remaining debt is the large returned bundles, plus the local relative-velocity/standoff assembly and local-vs-shell comparison workflows that still sit in a deep layer. Code TODO: existing TODO(debt) + TODO(griblet).
