...
- `starwinds_analysis/physics/orbit_local.py` — **Debt**. Quantity-specific local orbit workflow wrappers (`local_mass_loss_*`, `local_torque_*`) now use generic shell primitives from `analysis.shells`, but the file still assembles orbit comparison workflows in a deep layer. SI field requests now go through SmartDs/griblet (including direct sampled `U_r/U_a/B_r/B_a`; no `resolve_*` in this file). Code TODO: existing TODO(debt).
