...
- `starwinds_analysis/physics/orbit_local.py` — **Debt**. This file now consumes sampled curves only and no longer constructs orbit geometry. It still assembles local-vs-shell comparison workflows in a deep layer and still returns large comparison bundles. SI field requests go through SmartDs/griblet flux and torque-density fields; remaining debt is the workflow shape, not raw field reconstruction. Code TODO: existing TODO(debt).
