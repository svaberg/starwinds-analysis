...
- `starwinds_analysis/analysis/trajectories.py` — **Reviewed**. Orbit geometry and 1D-curve sampling primitives live in `analysis`; removed the one-module `sampling/` package. Orbit sampling now returns a curve `SmartDs`, and the shared periodic-curve velocity helper lives here.
