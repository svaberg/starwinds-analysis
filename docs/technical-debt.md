...
- `starwinds_analysis/analysis/shells.py` — **Debt**. Both shell samplers now return structured `SmartDs`, but temporary compatibility attributes (`.radii/.theta/.phi/.x/.y/.z/.area/.fields`) are still attached for legacy callers/tests. Dead `resolve_*` helpers and the `SphericalShellSamples` custom container were removed. Code TODO: existing TODOs.
