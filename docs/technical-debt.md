...
- `starwinds_analysis/physics/fluxes.py` — **Debt**. Quantity-specific shell flux profile wrappers remain (`*_vs_radius`); SI field requests now go through SmartDs/griblet and the shell `SmartDs` is read directly (no compat `.fields/.x/.area` usage), but local quantity recomputation (`B_r`, `U_r`, `E*U_r`) is still done in code. Code TODO: existing TODO(debt) + TODO(griblet).
...
- `starwinds_analysis/physics/mass_loss.py` — **Debt**. Quantity-specific shell profile wrapper (`mass_loss_vs_radius`) and `analysis.shells` dependency remain. The custom container (`ShellMassFluxMap`) and one-off shell-map wrapper were removed; tests/examples now use shell primitives directly. SI field requests now go through SmartDs/griblet and shell values are read directly from the shell `SmartDs` (no `resolve_*`, no compat `.fields/.x/.area` usage). Code TODO: added TODO + existing TODO(griblet).
...
- `starwinds_analysis/physics/shell_torque.py` — **Debt**. Quantity-specific shell torque profile wrapper (`torque_vs_radius`) depends on `analysis.shells`; SI field requests now go through SmartDs/griblet and shell values are read directly from the shell `SmartDs` (no `resolve_*`, no compat `.fields/.x/.area` usage). Code TODO: added TODO.
