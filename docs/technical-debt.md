...
- `starwinds_analysis/physics/orbit_pressure.py` — **Debt**. This file now consumes a sampled curve `SmartDs` only and no longer constructs orbit geometry. Remaining debt is the large returned summary bundle and the local relative-velocity/standoff assembly that still sits outside griblet. Code TODO: added TODO + existing TODO(griblet).
- `starwinds_analysis/physics/orbit_local.py` — **Debt**. Quantity-specific local orbit workflow wrappers (`local_mass_loss_*`, `local_torque_*`) now use generic shell primitives from `analysis.shells`, and the circular wrappers are now thin `e=0` forwards. The file still assembles orbit comparison workflows in a deep layer and still returns large comparison bundles. SI field requests now go through SmartDs/griblet flux and torque-density fields; remaining debt is the workflow shape, not raw field reconstruction. Code TODO: existing TODO(debt).
