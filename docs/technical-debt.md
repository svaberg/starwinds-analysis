...
- `starwinds_analysis/physics/mass_loss.py` — **Debt**. Quantity-specific shell profile wrapper (`mass_loss_vs_radius`) and `analysis.shells` dependency remain. The custom container (`ShellMassFluxMap`) and one-off shell-map wrapper were removed; tests/examples now use shell primitives directly. SI field requests now go through SmartDs/griblet (no `resolve_*` in this file). Code TODO: added TODO + existing TODO(griblet).
